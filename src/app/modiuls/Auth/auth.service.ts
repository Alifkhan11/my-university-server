...
const changePasswordfromDB=async( payloads: JwtPayload,userPassword: { oldPassword: string; newPassword: string })=>{
    ...
    if(isdelete){
      ...
      //cheak password 
    const isPasswordMatched=await bcrypt.compare(userPassword.oldPassword,user.password)
    if(!isPasswordMatched){
        throw new AppError(httpStatus.NOT_FOUND,'This Password wrong')
    }


    const newHashedPassword=await bcrypt.hash(userPassword.newPassword,Number(config.BCRYPT_SALT_ROUND))

    await User.findOneAndUpdate({id:payloads.userId,role:payloads.role},{
